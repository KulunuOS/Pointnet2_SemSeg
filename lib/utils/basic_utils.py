...
intrinsic_matrix = {
    ...
    'pascal': np.asarray([[-3000.0, 0.0,    0.0],
                         [0.0,      3000.0, 0.0],
                         [0.0,      0.0,    1.0]]),
    'ycb_K1': np.array([[615.1397026909722, 0.0, 322.78683465719223], [0.0, 615.4323641853988, 244.22407517651774], [0.0, 0.0, 1.0]], np.float32),
...
class Basic_Utils():
    ...
    def draw_bounding_box(self, img, corner):
        img = cv2.line(img, tuple(corner[0]), tuple(corner[1]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[0]), tuple(corner[2]), (0,255,255),1)
        img = cv2.line(img, tuple(corner[2]), tuple(corner[3]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[3]), tuple(corner[1]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[4]), tuple(corner[5]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[5]), tuple(corner[7]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[6]), tuple(corner[2]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[3]), tuple(corner[7]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[6]), tuple(corner[2]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[4]), tuple(corner[0]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[4]), tuple(corner[6]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[6]), tuple(corner[7]), (0,255,255), 1)
        img = cv2.line(img, tuple(corner[1]), tuple(corner[5]), (0,255,255), 1)
        return img
    ...
    def get_pointxyz(
        ...
        if ds_type == "ycb":
            ...
            ptxyz_pth = os.path.join('/home/kulunu/PVN3D/pvn3d/datasets/Adapt/Adapt_dataset/models','obj_'+str(cls)+'.ply')
            #pointxyz = self.ply_vtx(ptxyz_pth)
            pointxyz = np.asarray(pcl.load(ptxyz_pth))
            dellist = [j for j in range(0, len(pointxyz))]
            dellist = random.sample(dellist, len(pointxyz) - 2000)
    ...
    def get_pointxyz_cuda(
        ...
        elif ds_type == "openDR" or ds_type == "CrankSlider" or ds_type == "Adapt" :
    ...
    def get_kps(
        ...
        if type(cls) is int:
            if ds_type == 'ycb':
                ...
                kps = np.loadtxt(self.config.Adapt_kps_dir+'/{}/{}.txt'.format(cls+1,kp_type), dtype=np.float32)
                return kps.copy()
            else:
    ...
    def get_ctr(self, cls, ds_type):
        ...
        if ds_type == "ycb":
            ...
            cor_pattern = os.path.join(
                self.config.Adapt_kps_dir, '{}/corners.txt'.format(cls+1),
            )
            cors = np.loadtxt(cor_pattern.format(cls+1), dtype=np.float32)
            ctr = cors.mean(0)
